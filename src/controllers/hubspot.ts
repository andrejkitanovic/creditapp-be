...
export const hsUpdateContact = async (
	...
	try {
		const object = filterObject({
			...
			// email: customer.email,
			// date_of_birth: customer.birthday && dayjs(customer.birthday).utc().startOf('day').toDate(),
			// address: customer.address,
			// city: customer.city,
			// state: customer.state,
			// zip: customer.zip,
			// phone: customer.phone ? ["+1",customer.phone].join("") : undefined,
			// mobilephone:customer.mobilePhone ? ["+1",customer.mobilePhone].join("") : undefined,
			...
			// dl_issuance_date: customer.personalInfo.driversLicenseIssueDate && dayjs(customer.personalInfo.driversLicenseIssueDate).utc().startOf('day').toDate(),
			// dl_expiration_date: customer.personalInfo.driversLicenseExpireDate && dayjs(customer.personalInfo.driversLicenseExpireDate).utc().startOf('day').toDate(),
			...
			// CALCULATED INCOME — only these fields sync to HubSpot
			monthly_gross_income: customer.employmentInfo.monthlyGrossIncome,
			// annual_personal_income: customer.employmentInfo.annualPersonalIncome, - CALCULATED PROPERTY IN HS, CANNOT BE SET
